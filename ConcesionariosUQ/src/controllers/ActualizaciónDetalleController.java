...
public class ActualizaciónDetalleController implements Initializable{
    ...
    void actualizarTransaccion(ActionEvent event) {
    	...
    	if(codigoEmpleado!=null && !codigoEmpleado.equals("")){
    		crearFactura(marca, modelo, codigo, codigoEmpleado, transaccion, valor, fecha, adminViewController.getTransaccionSelection().getCar());
    		this.stage.close();
	...
	private void crearFactura(String marca, String modelo, String codigo, String codigoEmpleado,
		...
		String flag = singleton.actualizarFactura(marca, modelo, codigo, codigoEmpleado, transaccion, valor, fecha, car);
		if (flag == "0") {
			...
			mostrarMensaje("Información", "Factura no actualizada", "El ID del empleado está errado", AlertType.INFORMATION);
		}
	}
